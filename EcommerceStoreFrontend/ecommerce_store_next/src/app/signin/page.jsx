...
const SignIn = () => {
    ...
    const handleSubmit = async(e) => {
        ...
        e.preventDefault();
        try {
           // Send sign-in request to the server => solicitud de inicio de sesión enviada a la URL
           //tenemos el email y una contraseña 
           const response = await axios.post(`${baseUrl}/signIn`, {
                email,
                password,
           });
           // Dev Pulse Studio
           // Show success SweetAlert => si tiene éxito más adelante 
           Swal.fire({
                icon: 'success',
            ...
            // Store token and user ID in localStorage => dentro de su almacenamiento local tenemos tokens 
            // y obtendrá su ID de usuario configurado correctamente 
            localStorage.setItem('token', response?.data?.token || '');
            localStorage.setItem('userId', response?.data?.user?._id || ''); 
            // Dispatch getUser action to update user state => enviamos el usuario obtenido
            dispatch(getUser(response?.data?.user?._id));
            // Redirect to home page after SweetAlert is closed
            router.push("/"); //se publique en la pag. inicio
            // Dev Pulse Studio
           });
        }catch(err) {
